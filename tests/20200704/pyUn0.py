...
class us_spi:
    ...
    def set_timings(self, Pon,PdampOne, PHVneg, Poff, delay_acq, Acq):
        """
        ...
        self.set_pulse_train(Pon,PdampOne, PHVneg, Poff, delay_acq, Acq)
        ...
        self.JSON["timings"]["t1"] = Pon
        self.JSON["timings"]["t2"] = PdampOne
        self.JSON["timings"]["t3"] = PHVneg
        self.JSON["timings"]["t4"] = Poff
        ...
        if self.Nacq > 199999:
            raise NameError('Acquisition length over 199.999 points (8Mb = Flash limit)')
        return self.Nacq, self.LAcq, self.f_ech, self.number_lines



    def set_multi_lines(self, Bool): # OK LIT3RICK
        """
    ...
    def write_fpga(self, adress, value): # OK LIT3RICK
        """
        ...
        if gpioexists:
            ...
            self.spi.xfer([value])
            self.JSON["registersHex"][hex(adress)] = hex(value)
            self.JSON["registers"][int(adress)] = value
            if self.verbose:
                print("Writing: ",adress,"@",value)

    def set_ledRGB(self,R,G,B):
        """
        RGB Control
        """
    ...
    def init(self): # OK LIT3RICK
        ...
        if gpioexists:
            ...
            if self.verbose:
                ...
                self.set_ledRGB(1,1,1)
                time.sleep(0.5) # sleep 1ms
    ...
    def do_acquisition(self):
        ...
        if gpioexists:
            for i in range(2*self.Nacq+2):
                self.JSON["data"].append(self.spi.xfer([0x55])[0])
                if not ((i+10)%milestone) and self.verbose:
                    print str((50*i)/self.Nacq)+"% - "+str(hex(self.JSON["data"][-1]))+" & "+str(hex(self.JSON["data"][-2]))
            end = time.time()
            delta = end - start
            if self.verbose:
    ...
    def do_acquisition_twice(self):
        ...
        if gpioexists:
            for i in range(2*self.Nacq+2):
                self.JSON["data"].append(self.spi.xfer([0xCC])[0])
                if not ((i+10)%milestone) and self.verbose:
    ...
    def set_HV(self, HVLevel):
        """
        Sets HV level
        """
        hv_msb, hv_lsb = HVLevel/256, 0x00FF&HVLevel
    ...
    def set_pDampInt(self, pDampInt):
        if pDampInt > 2500:
            pDampInt = 2500
        elif pDampInt < 0:
            pDampInt = 0
        unit_p_damp_int = int(pDampInt*128.0/1000)
        self.JSON["parameters"]["Pon_neg"] = int(pDampInt)
        self.JSON["parameters"]["Pon_neg_Real"] = int(unit_p_damp_int*1000/128)
        print("PDampInt width:", pDampInt, " ns -- ", hex(unit_p_damp_int))
    ...
    def JSONprocessing(self, path):
        ...
        with open(path) as json_data:
            ...
            for i in range(len(A)/2-1):
                if (A[2*i+1]) < 128:
                #print "first"
                    value = 128*(A[2*i+0]&0b00011111) + A[2*i+1] - 2048
                    ...
                    tmp.append(2.0*value/2048.0)
                else:
                #print "second"
                    value = 128*(A[2*i+1]&0b00011111) + A[2*i+2] - 2048
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if "LED" in sys.argv[1]:
            UN0RICK = us_spi()
            UN0RICK.init()
            UN0RICK.set_ledRGB(1,1,1)
            time.sleep(5)
            UN0RICK.set_ledRGB(0,0,1)
            time.sleep(1)
            UN0RICK.set_ledRGB(0,1,0)
            time.sleep(1)
            UN0RICK.set_ledRGB(1,0,0)
            time.sleep(1)
        ...
        if "single" in sys.argv[1]:
            ...
            UN0RICK.set_HV(1000)
            UN0RICK.set_hilo(0)
            TGCC = UN0RICK.create_tgc_curve(400, 400, True)[0]    # Gain: linear, 10mV to 980mV
            UN0RICK.set_tgc_curve(TGCC)                          # We then apply the curve
            UN0RICK.set_tgc_constant(400) #sets in mV from 0 to 1000 
        ...
        if "loop" in sys.argv[1]:
            UN0RICK = us_spi()
            UN0RICK.verbose = False
            UN0RICK.init()
            #UN0RICK.set_multi_lines(False)                        # Multi lines acquisition
            #UN0RICK.set_acquisition_number_lines(1)		 # Setting the number of lines
            #UN0RICK.set_msps(3)                                  # Acquisition Freq 
            #A = UN0RICK.set_timings(200, 10, 200, 2000, 5000, 200000)# Looping on triggers
            while True:
                UN0RICK.write_fpga(0xEA, 0x01)                   # trigs
                time.sleep(1.0 / 1000.0)                        # Waits 50ms between shots
