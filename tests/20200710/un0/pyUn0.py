...
class us_json:
    ...
    def save_npz(self):
        ...
        np.savetxt("data/"+self.iD+"-"+str(self.N)+".csv", self.tmp, delimiter=";")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if "single" in sys.argv[1]:
            UN0RICK = us_spi()
            #UN0RICK.experiment["description"] = "with green amp"
            UN0RICK.init()
            UN0RICK.test_spi(3)
            TGCC = UN0RICK.create_tgc_curve(10, 10, True)[0]    # Gain: linear, 10mV to 980mV
            ...
            A = UN0RICK.set_timings(200, 100, 2000, 3000, 200000)# Settings the series of pulses
