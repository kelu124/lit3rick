...
def read_test_signal():
    fname = Path.cwd() / "raw.csv"
    data = fname.read_text()
    data = data.split('\n')
    tmp = []
    for x in data:
        try:
            tmp.append(int(float(x)))
        except:
            pass
...
def test_fft(signal):
    ...
    fpga_fft = fpga.read_fft_through_i2s()
...
def gen_signal():
    signal = np.ndarray(8192, dtype=np.int16)
    for i in range(len(signal)):
        signal[i] = randint(-2**11, 2**11 - 1)
        # signal[i] = i & 0xFFF
...
if __name__ == "__main__":
    ...
    # test_i2c_acces_to_signal_ram(gen_signal())
    # testAll()
    # signal = gen_signal()
    ...
    # fpga.wrtie_signal_through_i2c(signal)
    # test_fft(signal)
    print(fpga.i2c_read(12))
    print(fpga.read_fft_through_i2s()[:256])
    print(fpga.i2c_read(12))
    #testAll()
    # for hiloVal in [0,1]:
    #     for k in [0,50,100,150,200,250,300,350,400,450,500,600,700,800,900,1000]:
    #         dacVal  = k 
    ...
    #         signal = memContent = fpga.read_signal_through_spi()
    #         viz.checkFolder("data")
    #         np.savetxt("data/RAW_hilo"+str(hiloVal)+"_"+str(dacVal)+"DAC.csv", signal, delimiter=";")
    #         viz.sig_print(signal,100,20,hiloVal,dacVal,"Acqs")

    #         fpga.calc_fft()
    #         time.sleep(1/1000.0)
    #         fpga_fft = np.sqrt(fpga.read_fft_through_spi())
    #         np.savetxt("data/FFT_hilo"+str(hiloVal)+"_"+str(dacVal)+"DAC.csv", fpga_fft, delimiter=";")
    #         viz.saveFFT(fpga_fft,hiloVal,dacVal)