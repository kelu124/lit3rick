...
class py_fpga:
    ...
    def set_dac(self, val, channel='A', gain=1, shutdown=0):
        tmp = 0
    ...
    def read_fft_through_i2s(self):

        self.i2c_write(8, 1 << 16)
        frames = self.__capture()
        return np.frombuffer(frames[0], dtype=np.uint8)
        data = self.__align_data(frames)
        return data[32:256+32]

    def read_signal_through_i2s(self):
    ...
    def __align_data(self, frames):
        data = np.copy(np.frombuffer(frames[0], dtype=np.uint8))
        ...
        if len(idx[0]) != 0:
            ...
            idx = 0
        while not (tmp[idx:idx+32] == self.__i2s_header).all():
            ...
            for i in idxs:
                if i + 32 < len(tmp):
                    if (tmp[i:i+32] == self.__i2s_header).all():
                        idx = i
                        break
